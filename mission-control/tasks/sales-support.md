# Sales Support Tasks — Rankingsb
## Sal Vasquez Enablement Tracking

---

## Rep Info
- **Name:** Sal Vasquez
- **Phone:** 805-724-2788
- **Email:** vasquez.salvador.jr@gmail.com
- **Start Date:** Feb 17, 2026
- **Commission:** $997 per close (50% of setup fee)

---

## Current Status: Week 2

### Week 1 Results (Feb 17-18)
- [ ] Calls made: ___
- [ ] Conversations: ___
- [ ] Audits scheduled: ___
- [ ] Audits completed: ___
- [ ] Deals closed: ___
- **Status:** Awaiting Week 1 results from Sal (Critical)

### Week 2 Deliverables (COMPLETE)
- [x] Week 2 packet created (HVAC focus)
- [x] 20 HVAC leads researched
- [x] 7 audit snippets created
- [x] Calling schedule prepared
- [x] Tracking sheet ready

### Week 3 Deliverables (COMPLETE)
- [x] Week 3 packet created (Plumbing focus)
- [x] 8 new audit snippets added (total: 15)
- [x] Plumbing-specific talk tracks
- [x] Emergency service angles
- [x] Commercial/property manager scripts

### Week 4 Deliverables (COMPLETE - Feb 20)
- [x] Week 4 packet created (Roofing focus)
- [x] 20 roofing leads researched (SB + Ventura)
- [x] 5 roofing-specific audit snippets
- [x] Storm season angle scripts
- [x] Insurance/commercial property manager angles
- [x] Emergency service hooks

---

## Deliverables Log

| Date | Deliverable | Status | File |
|------|-------------|--------|------|
| Feb 17 | Week 1 packet (Electricians) | ✅ Delivered | Week 1 packet |
| Feb 17 | Audit Generator CLI | ✅ Shipped | GHL integrated |
| Feb 17 | 10 electrician leads | ✅ Loaded | GHL pipeline |
| Feb 17 | Daily tracker | ✅ Provided | GHL dashboard |
| Feb 18 | Week 2 packet (HVAC) | ✅ Created | `SAL-WEEK-2-PACKET.md` |
| Feb 18 | 20 HVAC leads | ✅ Researched | Spike report |
| Feb 18 | Audit Snippets Library | ✅ Created | `AUDIT-SNIPPETS-LIBRARY.md` |
| Feb 19 | Week 2 packet | ✅ Delivered | Via email/SMS |
| Feb 19 | Week 3 packet (Plumbing) | ✅ Created | `SAL-WEEK-3-PACKET.md` |
| Feb 19 | Additional audit snippets | ✅ Added | 15 total snippets |
| Feb 19 | Emergency/commercial angles | ✅ Created | HVAC + Plumbing focus |
| Feb 20 | Week 4 packet (Roofing) | ✅ Created | `SAL-WEEK-4-PACKET.md` |
| Feb 20 | 20 Roofing leads | ✅ Researched | SB + Ventura counties |
| Feb 20 | 5 Roofing audit snippets | ✅ Created | Storm + insurance angles |

---

## Weekly Packet Schedule

| Week | Focus Industry | Lead Count | Status |
|------|---------------|------------|--------|
| 1 | Electricians | 20 | ✅ Complete |
| 2 | HVAC | 20 | ✅ Complete |
| 3 | Plumbing | 20 | ✅ Complete |
| 4 | Roofing | 20 | ✅ Ready (Feb 24) |
| 5 | Solar | 20 | ⏳ Pending |
| 6 | Appliance Repair | 20 | ⏳ Pending |

---

## Sales Collateral Inventory

### Scripts
- [x] Cold Call Script (Audit-first approach)
- [x] Audit Review Call Script
- [x] Voicemail Script
- [x] Email Follow-Up Templates
- [x] Objection Handling Guide
- [x] 15 Audit Snippets (industry-specific angles)
- [x] Emergency service scripts
- [x] Commercial/property manager scripts
- [x] Spanish market scripts

### Tools
- [x] Audit Generator (GHL integrated)
- [x] Lead Research System (Spike agent)
- [x] Commission Tracker
- [x] Daily Activity Tracker

### Training
- [x] Initial onboarding (Feb 17)
- [x] GHL training
- [x] Script review
- [x] Week 2 briefing (scheduled)

---

## Next Actions

### Immediate (Today)
- [ ] Get Sal's Week 1-2 results
- [ ] Deliver Week 3 packet to Sal
- [ ] Schedule Week 3 kickoff call

### This Week
- [x] Check in with Sal daily (5 min)
- [ ] Review audit quality
- [ ] Celebrate first close (when it happens)
- [x] Prep Week 4 packet (Roofing) ✅ DONE

### Next Week (Feb 24-28)
- [ ] Deliver Week 4 packet to Sal
- [ ] Schedule Week 4 kickoff call (storm season angle)
- [ ] Check in daily
- [ ] Prep Week 5 (Solar) — pending

### Ongoing
- [ ] Get Sal's Week 1-3 results (still critical)

---

## Blockers
- None currently
- Awaiting Sal's feedback from Week 1

---

*Last updated: Feb 19, 2026 2:45 PM*
*Next review: Feb 20, 2026 9 AM*
