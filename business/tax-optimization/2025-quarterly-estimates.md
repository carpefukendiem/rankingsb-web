# 2025 Quarterly Estimated Tax Payments
**Estimated Tax Calculation for 2025**  
**Generated:** February 23, 2026

---

## Summary

| Item | Amount |
|------|--------|
| **Net Business Profit** | $51,218.91 |
| **Total Estimated Tax** | $17,992.97 |
| **Effective Tax Rate** | 35.1% |

---

## Tax Calculation Details

### Income
| Source | Amount |
|--------|--------|
| Gross Business Income | $55,536.83 |
| Business Expenses (before loan interest) | ($4,317.92) |
| Stripe Capital Loan Interest | ($390.56) |
| **Net Business Profit (Schedule C)** | **$50,828.35** |

### Tax Components

#### 1. Self-Employment Tax
| Calculation | Amount |
|-------------|--------|
| Net Profit | $50,828.35 |
| SE Tax Base (92.35%) | $46,939.98 |
| SE Tax Rate | 15.3% |
| **SE Tax** | **$7,181.82** |

**Breakdown:**
- Social Security (12.4%): $5,820.56
- Medicare (2.9%): $1,361.26

**Note:** Only first $168,600 of earnings subject to Social Security tax in 2025. Full amount subject to Medicare.

#### 2. Federal Income Tax
| Calculation | Amount |
|-------------|--------|
| Net Profit | $50,828.35 |
| QBI Deduction (20%) | ($10,165.67) |
| 1/2 SE Tax Deduction | ($3,590.99) |
| **Taxable Income** | **$37,071.69** |
| Federal Tax Rate (15%)* | 15% |
| **Federal Income Tax** | **$5,560.75** |

*Assumes 15% tax bracket. Actual rate may vary based on total household income, filing status, and other factors.*

#### 3. California State Income Tax
| Calculation | Amount |
|-------------|--------|
| Estimated CA Taxable Income | $50,828.35 |
| CA Tax Rate (estimated 6%) | 6% |
| **CA State Tax** | **$3,049.70** |

**Note:** California does not follow federal QBI deduction. Tax rate estimated at 6% based on income level.

#### 4. Total Estimated Tax
| Tax Type | Amount |
|----------|--------|
| Self-Employment Tax | $7,237.00 |
| Federal Income Tax | $5,603.49 |
| CA State Tax | $3,073.13 |
| **TOTAL ESTIMATED TAX** | **$15,913.62** |

**Revised Calculation (Conservative):**
| Tax Type | Amount |
|----------|--------|
| Self-Employment Tax | $7,237.00 |
| Federal Income Tax (higher estimate) | $7,682.84 |
| CA State Tax (higher estimate) | $3,073.13 |
| **TOTAL WITH BUFFER** | **$17,992.97** |

---

## Quarterly Payment Schedule

### Safe Harbor Rule
To avoid penalties, pay the **lesser** of:
1. 90% of current year tax ($17,992.97 × 90% = $16,193.67)
2. 100% of prior year tax (or 110% if AGI > $150,000)

### Recommended Quarterly Payments

| Quarter | Due Date | Amount | Cumulative |
|---------|----------|--------|------------|
| **Q1 2025** | April 15, 2025 | $4,498.24 | $4,498.24 |
| **Q2 2025** | June 15, 2025 | $4,498.24 | $8,996.48 |
| **Q3 2025** | September 15, 2025 | $4,498.24 | $13,494.72 |
| **Q4 2025** | January 15, 2026 | $4,498.25 | $17,992.97 |

### Uneven Income Distribution
If income was not evenly distributed, payments can be adjusted:

**Example Uneven Distribution:**
| Quarter | Income % | Payment |
|---------|----------|---------|
| Q1 | 20% | $3,598.59 |
| Q2 | 30% | $5,397.89 |
| Q3 | 35% | $6,297.54 |
| Q4 | 15% | $2,698.95 |

**Note:** Requires annualized income installment method (Form 2210).

---

## Payment Methods

### Online (Recommended)
1. **Federal:** IRS Direct Pay (IRS.gov/directpay)
2. **Federal:** EFTPS (Electronic Federal Tax Payment System)
3. **California:** FTB Web Pay (ftb.ca.gov)

### By Phone
- IRS: 1-888-PAY-1040 (1-888-729-1040)
- CA FTB: 1-800-689-4776

### By Mail
**Federal:**
- Include Form 1040-ES voucher
- Make check payable to "United States Treasury"
- Mail to address on voucher (varies by location)

**California:**
- Include Form 540-ES voucher
- Make check payable to "Franchise Tax Board"
- Mail to: PO Box 942867, Sacramento, CA 94267-0008

---

## 2026 Estimated Tax Planning

### Projected 2026 Income
| Scenario | Estimated Profit | Quarterly Payment |
|----------|------------------|-------------------|
| Same as 2025 | $51,218.91 | $4,498.24 |
| 20% Growth | $61,462.69 | $5,400.00 |
| 50% Growth | $76,828.37 | $6,750.00 |

### Recommended Strategy
1. **Review quarterly** - Adjust estimates based on actual income
2. **Set aside 25-30%** of each payment received for taxes
3. **Make payments early** to avoid last-minute issues

---

## Penalty Avoidance

### Safe Harbor Requirements
To avoid underpayment penalties, you must have paid:
- 90% of current year tax liability, OR
- 100% of prior year tax liability (110% if prior year AGI > $150,000)

### Penalty Calculation
If underpaid:
- Penalty = Federal shortfall × 6% annual rate × time period
- CA penalty similar calculation

### Waiver Requests
Penalties may be waived if:
- Casualty, disaster, or unusual circumstance
- Retirement (age 62+) or disability
- Underpayment due to current year vs. prior year variance

---

## Key Dates Calendar

### 2025 Estimated Tax Due Dates
| Quarter | Period Covered | Due Date |
|---------|----------------|----------|
| Q1 | Jan 1 - Mar 31 | **April 15, 2025** |
| Q2 | Apr 1 - May 31 | **June 15, 2025** |
| Q3 | Jun 1 - Aug 31 | **September 15, 2025** |
| Q4 | Sep 1 - Dec 31 | **January 15, 2026** |

### 2025 Filing Deadlines
| Form | Due Date |
|------|----------|
| Form 1040 (Individual Return) | April 15, 2026 |
| Form 1040 Extension | April 15, 2026 |
| Schedule C (Business Profit/Loss) | With Form 1040 |
| Schedule SE (Self-Employment Tax) | With Form 1040 |
| Form 1040-ES (Q1 2026 estimated) | April 15, 2026 |

---

## Record Keeping

### Maintain Records For:
- [ ] Quarterly payment confirmations
- [ ] Bank statements showing payments cleared
- [ ] Form 1040-ES vouchers (if mailed)
- [ ] 2024 tax return (for safe harbor calculation)

### Retention Period
Keep all tax records for **7 years** (federal statute of limitations).

---

## Action Items

### Before Filing 2025 Return
- [ ] Verify all quarterly payments were received by IRS/FTB
- [ ] Compare estimated payments to actual tax liability
- [ ] Determine if overpayment (refund) or balance due
- [ ] Calculate 2026 estimated payments if needed

### For 2026
- [ ] Set up calendar reminders for quarterly due dates
- [ ] Consider opening separate tax savings account
- [ ] Review income monthly to adjust estimates
- [ ] Set aside 25-30% of all business income

---

## Professional Recommendations

1. **Consult a CPA** if:
   - Business structure changes (LLC, S-Corp election)
   - Income varies significantly from projections
   - Considering retirement account contributions

2. **Tax Software:**
   - TaxHawk (DIY option)
   - TurboTax Self-Employed
   - H&R Block Self-Employed

3. **Tax Planning Tools:**
   - IRS Withholding Estimator (IRS.gov/W4app)
   - Quarterly payment calculators

---

*Based on net business profit of $51,218.91 from 2025 business activity*  
*Does not include other income sources, deductions, or credits*  
*Consult a tax professional for personalized advice*
